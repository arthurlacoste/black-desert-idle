...
-- Rend le boss mondial du PLANNING (Kzarka ET Vell aux horaires fixes) réellement partagé entre
-- tous les joueurs : PV communs (live_boss) et présence visible dans l'arène, exactement comme un
-- spawn admin. Avant le 2026-07-06, seul un spawn admin utilisait live_boss ; le planning
-- hebdomadaire restait une instance SOLO calculée par joueur.
-- Correctif du 2026-07-08 (voir supabase/migrations/20260708210000_ensure_scheduled_boss_include_vell.sql) :
-- Vell n'était pas dans la liste (boss_id/hp restaient figés sur Kzarka), donc ses 2 créneaux
-- hebdomadaires (jeudi 12h00, dimanche 16h45) ne créaient jamais de ligne partagée — chaque joueur
-- combattait Vell seul, sans voir les autres. boss_id/hp sont désormais dynamiques par occurrence.
-- La condition de réécriture ne compare plus spawned_at (qui pouvait faire écraser à tort un spawn
-- admin d'un AUTRE boss déjà en cours) : on ne réclame le créneau que si rien de valide n'est actif.
...
-- tout le monde, sans jamais écraser un spawn (admin ou planifié) déjà valide.
...
begin
  ...
  for v_entry in
    select * from (values
      (-1, 12, 45, 'kzarka', 400000::numeric), -- doit rester égal à BOSS_ROSTER.kzarka.hp côté client
      (-1, 19, 45, 'kzarka', 400000::numeric),
      (-1, 23, 45, 'kzarka', 400000::numeric),
      (0,  15, 45, 'kzarka', 400000::numeric),
      (6,  15, 45, 'kzarka', 400000::numeric),
      (4,  12, 0,  'vell',   550000::numeric),  -- doit rester égal à BOSS_ROSTER.vell.hp côté client
