...
/** Composant React — détail dépliable d'une session AFK passée (niveau avant→après, XP gagné, objets ramassés). Affiché quand on clique sur une ligne de l'historique. */
...
/** Composant React racine du modal "Bon retour" (progression niveau/silver/XP animée, objets ramassés, historique AFK filtrable par palier, chaque session dépliable au clic pour son détail). */
                      ...
                      return h('div', {
                        ...
                        h('div', {
                          ...
                          h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 } },
                            // chevron : pivote de 90° quand la session est dépliée (affordance "cliquable")
...
async function reconnectModalLoadHistory() {
  ...
  try {
    ...
    const history = rows.map(r => ({
      ...
      // détail dépliable au clic (2026-07-15) : get_afk_history renvoie déjà setof
      // player_afk_sessions (toutes les colonnes), on mappe juste les champs qui n'étaient pas
      // encore utilisés -- aucun aller-retour serveur supplémentaire.
      xp: r.xp_gained || 0,
